...
*/
package com.metacarta.crawler.connectors.filenet;
...
public class DocClassSpec
{
	...
	public DocClassSpec(SpecificationNode sn)
	{
		...
		String allmetadata = sn.getAttributeValue(com.metacarta.crawler.connectors.filenet.FilenetConnector.SPEC_ATTRIBUTE_ALLMETADATA);
		...
		if (allmetadata.equals("false"))
		{
			...
			while (j < sn.getChildCount())
			{
				...
				if (node.getType().equals(com.metacarta.crawler.connectors.filenet.FilenetConnector.SPEC_NODE_METADATAFIELD))
				{
					String fieldName = node.getAttributeValue(com.metacarta.crawler.connectors.filenet.FilenetConnector.SPEC_ATTRIBUTE_VALUE);
		...
		{
			SpecificationNode node = sn.getChild(j++);
			if (node.getType().equals(com.metacarta.crawler.connectors.filenet.FilenetConnector.SPEC_NODE_MATCH))
			{
				String matchTypeString = node.getAttributeValue(com.metacarta.crawler.connectors.filenet.FilenetConnector.SPEC_ATTRIBUTE_MATCHTYPE);
				String matchField = node.getAttributeValue(com.metacarta.crawler.connectors.filenet.FilenetConnector.SPEC_ATTRIBUTE_FIELDNAME);
				String matchValue = node.getAttributeValue(com.metacarta.crawler.connectors.filenet.FilenetConnector.SPEC_ATTRIBUTE_VALUE);
