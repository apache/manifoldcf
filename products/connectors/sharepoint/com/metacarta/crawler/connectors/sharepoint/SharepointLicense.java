 ...
 * verify() to figure out if the sharepoint connector is enabled.
 * TBD: replace this with a more generic interface POST FREEZE.
 */

public class SharepointLicense {
    public static final String _rcsid = "@(#)$Id$";
    

    private static SharepointLicense _instance = null;

    private static synchronized void createInstance() {
	if (_instance == null) {
		_instance = new SharepointLicense();
	}
    }   

    public static SharepointLicense getInstance() {
	if (_instance == null) {
	    createInstance();
	}
	return _instance;
	...
	try
		...
		String connector_prefix = (String)constantsClass.getDeclaredField("SHAREPOINT_CONNECTOR_PREFIX").get(null);
