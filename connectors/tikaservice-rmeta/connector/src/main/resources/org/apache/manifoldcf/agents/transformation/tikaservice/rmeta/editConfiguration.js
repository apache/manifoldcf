...
function checkConfig()
{
  ...
  if (editconnection.connectionTimeout.value != "" && !isInteger(editconnection.connectionTimeout.value))
  {
    alert("$Encoder.bodyJavascriptEscape($ResourceBundle.getString('TikaExtractor.PleaseSupplyAValidNumber'))");
    editconnection.connectionTimeout.focus();
    return false;
  }
  if (editconnection.socketTimeout.value != "" && !isInteger(editconnection.socketTimeout.value))
  {
    alert("$Encoder.bodyJavascriptEscape($ResourceBundle.getString('TikaExtractor.PleaseSupplyAValidNumber'))");
    editconnection.socketTimeout.focus();
    return false;
  }
  if (editconnection.retryInterval.value != "" && !isInteger(editconnection.retryInterval.value))
  {
    alert("$Encoder.bodyJavascriptEscape($ResourceBundle.getString('TikaExtractor.PleaseSupplyAValidNumber'))");
    editconnection.retryInterval.focus();
    return false;
  }
  return true;
}
