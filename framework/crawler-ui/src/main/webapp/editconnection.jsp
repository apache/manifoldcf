...
<script type="text/javascript">
  ...
  $.ManifoldCF.setTitle('<%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.ApacheManifoldCFEditConnection")%>',
...
<div class="row">
  <div class="col-md-12">
    ...
    <div class="callout callout-warning">
      <p><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.NoRepositoryConnectorsRegistered")%>
      </p>
    </div>
    <%} else {%>
      ...
      <form class="standardform" name="editconnection" action="execute.jsp" method="POST" enctype="multipart/form-data">
        ...
        <div class="box-body">
          <ul class="nav nav-tabs" role="tablist">
            ...
            <%} else {%>
            <li>
              <a href="#tab_<%=tabNum%>" data-toggle="tab" alt='<%=org.apache.manifoldcf.ui.util.Encoder.attributeEscape(tab)+" "+Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.tab")%>' onclick='<%="javascript:SelectTab(\""+tab+"\");return false;"%>'>
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane active" id="tab_<%=activeTab%>">
              <div class="form-group">
                <label><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.NameColon")%>
                </label>
                <%
              ...
              <div class="form-group">
                <label for="description"><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.DescriptionColon")%>
                </label>
            ...
            <div class="tab-pane active" id="tab_<%=activeTab%>">
              <div class="form-group">
                <label><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.ConnectionTypeColon")%>
                </label>
                <%
                  if (className.length() > 0) {
              ...
              <div class="form-group">
                <label><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.AuthorityGroupColon")%>
                </label>
            ...
            <div class="tab-pane active" id="tab_<%=activeTab%>">
              <div class="form-group">
                <label><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.MaxconnectionsColon")%>
                </label>
                <input type="text" size="6" name="maxconnections" class="form-control" value='<%=Integer.toString(maxConnections)%>'/>
              </div>
              <div class="form-group">
                <label><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.ThrottlingColon")%>
                </label>
                ...
                <table class="table table-bordered">
                  <tr>
                    <th>Action</th>
                    <th><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.BinRegularExpression")%>
                    </th>
                    <th><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.Description")%>
                    </th>
                    <th><%=Messages.getBodyString(pageContext.getRequest().getLocale(), "editconnection.MaxAvgFetchesMin")%>
                    </th>
                  </tr>
                  <%
                    int k = 0;
                  ...
                  <tr>
                    <td>
                      <input class="btn btn-danger btn-xs" type="button" value="<%=Messages.getBodyString(pageContext.getRequest().getLocale(),"editconnection.Delete")%>" alt='<%=Messages.getBodyString(pageContext.getRequest().getLocale(),"editconnection.Deletethrottle")+" "+org.apache.manifoldcf.ui.util.Encoder.attributeEscape(regexp)%>' onclick='<%="javascript:DeleteThrottle("+Integer.toString(k)+");"%>'/>
                    </td>
                    <td>
                  ...
                  <tr>
                    <td>
                      <input type="button" class="btn btn-success btn-xs" value="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.Add")%>" alt="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.Addthrottle")%>" onclick="javascript:AddThrottle();"/>
                    </td>
                    <td><input type="text" class="form-control" name="throttle" size="30" value=""/>
                    </td>
        ...
        <div class="box-footer clearfix">
          <div class="btn-group">
            ...
            <%if (className.length() > 0) {%>
            <input type="button" class="btn btn-primary" value="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.Save")%>" onClick="javascript:Save()" alt="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.SaveThisAuthorityConnection")%>"/>
            <%
            } else {
              if (tabName.equals(Messages.getString(pageContext.getRequest().getLocale(), "editconnection.Type"))) {
            %>
            <input type="button" class="btn btn-primary" value="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.Continue")%>" onClick="javascript:Continue()" alt="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.ContinueToNextPage")%>"/>
            <%
                }
              }
            %>
            <input type="button" class="btn btn-primary" value="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.Cancel")%>" onClick="javascript:Cancel()" alt="<%=Messages.getAttributeString(pageContext.getRequest().getLocale(),"editconnection.CancelConnectionEditing")%>"/>
          </div>
        </div>
      </form>
      <%}%>
