 ...
 * verify() to figure out if the jcifs connector is enabled.
 * TBD: replace this with a more generic interface POST FREEZE.
 */

public class SharedDriveLicense {
    public static final String _rcsid = "@(#)$Id$";
    

    private static SharedDriveLicense _instance = null;

    private static synchronized void createInstance() {
	if (_instance == null) {
		_instance = new SharedDriveLicense();
	}
    }   

    public static SharedDriveLicense getInstance() {
	if (_instance == null) {
	    createInstance();
	}
	return _instance;
	...
	*/
	...
	try
		...
		String connector_prefix = (String)constantsClass.getDeclaredField("SHARE_CONNECTOR_PREFIX").get(null);
		...
		throw new Error("Can't invoke license manager",e);
					   ...
					   // MetaCartaLicenseFileConstants.JCIFS_CONNECTOR_PREFIX);
    }
