...
public class AppSearchConfig extends AppSearchParam {
  ...
  final private static ParameterEnum[] CONFIGURATIONLIST
	  = {
	    ...
	    ParameterEnum.ENGINENAME,
	    ParameterEnum.SECRETAPIKEY,
      ...
      ParameterEnum.EXTRACTORPATTERN,
      ParameterEnum.REPLACEMENTSTRING
	  };

  /**
   * Build a set of AppSearchParameters by reading ConfigParams. If the value returned by ConfigParams.getParameter
   * is null, the default value is set.
   *
   * @param params
   */
  public AppSearchConfig(ConfigParams params) {
  ...
  /**
   * @return a unique identifier for one index on one AppSearch instance.
   */
  ...
  public final static String contextToConfig(IThreadContext threadContext, IPostParameters variableContext, ConfigParams parameters) throws ManifoldCFException {
    String rval = null;
    for (ParameterEnum param : CONFIGURATIONLIST) {
  ...
  final public String getEngineName() {
    return get(ParameterEnum.ENGINENAME);
  }

  final public String getSecretApiKey() {
    return get(ParameterEnum.SECRETAPIKEY);
  }
  ...
  final public String getExtractorPattern() {
    return get(ParameterEnum.EXTRACTORPATTERN);
  }

  final public String getReplacementString() {
    return get(ParameterEnum.REPLACEMENTSTRING);
  }
}
