...
import com.metacarta.agents.common.XMLStream;
...
*
*/
public class WebcrawlerConnector extends com.metacarta.crawler.connectors.BaseRepositoryConnector
{
...
/*
                                // Need a buffered reader so we can read lines at a time.
                                ...
                                try
                                {
                                        while (true)
                                        {
                                                ...
                                                if (line == null)
                                                        return;
                                                extractLinksFromLine(line,handler,tagMask);
                                        }
                                }
                                finally
                                {
...
/*
                ...
                throws MetacartaException
        {
                ...
                while (true)
                {
                        ...
                        if ((tagMask & TAGMASK_HREF) != 0)
                            tagPos = findTag(lowerLine,index,"href=",tagPos);
                        if ((tagMask & TAGMASK_SRC) != 0)
                            tagPos = findTag(lowerLine,index,"src=",tagPos);
                        if ((tagMask & TAGMASK_ACTION) != 0)
                            tagPos = findTag(lowerLine,index,"action=",tagPos);
                        
                        if (tagPos == -1)
                                return;
                        
                        if (tagPos >= line.length())
                                return;

                        char x = line.charAt(tagPos);
                        if (x == '"' || x == '\'')
                        {
                                tagPos++;
                                if (tagPos == line.length())
                                        return;
                                String delim = new String(new char[]{x});
                                int endPos = line.indexOf(delim,tagPos);
                                if (endPos == -1)
                                        endPos = line.length();
                                String rawURL = line.substring(tagPos,endPos);
                                // Destuff, since it's embedded in an html attribute
                                rawURL = attributeDecode(rawURL);
                                handler.noteDiscoveredLink(rawURL);
                                index = endPos+1;
                        }
                        else
                        {
                                // Don't look at tags without quotes.
                                index = tagPos;
                        }
                }
        }

        protected static int findTag(String line, int index, String match, int tagPos)
        {
                int position = line.indexOf(match,index);
                if (position == -1)
                        return tagPos;
                position += match.length();
                if (tagPos != -1 && tagPos < position)
                        return tagPos;
                return position;
        }
*/
