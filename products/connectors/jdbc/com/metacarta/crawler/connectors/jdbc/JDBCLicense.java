 ...
 * verify() to figure out if the jdbc connector is enabled.
...
public class JDBCLicense {
    ...
    private static JDBCLicense _instance = null;

    private static synchronized void createInstance() {
	if (_instance == null) {
		_instance = new JDBCLicense();
	}
    }   

    public static JDBCLicense getInstance() {
	if (_instance == null) {
	    createInstance();
	}
	return _instance;
	...
	try
		...
		String connector_prefix = (String)constantsClass.getDeclaredField("JDBC_CONNECTOR_PREFIX").get(null);
