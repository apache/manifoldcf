...
public class LiveLinkSupport
{
	...
	*/
		...
		throws MetacartaException
	{
		...
		{
			throw new MetacartaException("Connection failed: "+e.getMessage()+"; Details: "+llServer.getErrors(), e, MetacartaException.SETUP_ERROR);
		}
	}
	...
	*/
		...
		throws MetacartaException
	{
		...
		if (volID == null)
		{
			...
			if (volID == null)
				throw new MetacartaException("No such path: '"+LLpathString+"'");
			...
			LLValue objInfo = new LLValue().setAssocNotSet();
	            	...
	            	if ( status != 0 )
       	     		{
				throw new MetacartaException("Error adding document: "+llServer.getErrors());
			}
		...
		else
		{
			...
			int status = LLDocs.CreateVersion( volID.getVolumeID(),
				volID.getPathId(), filename, versionInfo );
	            	...
	            	if ( status != 0 )
       	     		{
				throw new MetacartaException("Error updating document: "+llServer.getErrors());
			}
	...
	*/
		...
		throws MetacartaException
	{
		...
		// Delete document
            	...
            	if ( status != 0 )
            	{
			throw new MetacartaException("Error deleting document: "+llServer.getErrors());
		}
	}
	...
	*/
		...
		throws MetacartaException
	{
		...
		if (catObjectID == -1)
			throw new MetacartaException("No such category: '"+categoryPathString+"'");
		...
		try
		{
			...
			if (status == 103101 || status == 107205)
				throw new MetacartaException("Can't fetch category attributes for category "+Integer.toString(catObjectID));
			...
			if (status != 0)
			{
				throw new MetacartaException("Error setting attribute value: "+Integer.toString(status)+": "+llServer.getErrors());
			}

			status = LLDocs.SetObjectAttributesEx(objIDValue,rvalue);
			// If either the object is wrong, or the object does not have the specified category, return null.
			if (status == 103101 || status == 107205)
				throw new MetacartaException("Can't set attributes for object "+Integer.toString(volID.getPathId())+
					" category "+Integer.toString(catObjectID));

			if (status != 0)
			{
				throw new MetacartaException("Error setting category version: "+Integer.toString(status)+": "+llServer.getErrors());
			}
		}
		catch (com.opentext.api.LLIllegalOperationException e)
		{
			throw new MetacartaException("Livelink: Unexpected illegal operation error: "+e.getMessage(),e);
		}
	...
	*/
		...
		throws MetacartaException
	{
		...
		try
		{
			...
			if (status != 0)
				throw new MetacartaException("Error getting user info: "+llServer.getErrors());
			...
			if (status != 0)
			{
				...
				if (status != 0)
					throw new MetacartaException("Error setting permissions: "+llServer.getErrors());
			}
		}
		...
		{
			throw new MetacartaException("Exception looking up user '"+userName+"'",e);
		}
		...
		throws MetacartaException
	{
		...
		while (charindex < startPath.length())
		{
			...
			try
			{
				...
				if (status != 0)
					throw new MetacartaException("Error finding category children: "+Integer.toString(status)+": "+llServer.getErrors());
				...
				else
				{
					...
					// illegal node.
					throw new MetacartaException("Error: no children!");
				}
			}
			catch (com.opentext.api.LLIllegalOperationException e)
			{
	...
	*/
	protected LLValue getObjectInfo(int vol, int id) throws MetacartaException
	{
		LLValue objinfo = new LLValue().setAssocNotSet();
		...
		if (status != 0)
			throw new MetacartaException("Error retrieving document object: "+llServer.getErrors());
	...
	*/		
		...
		throws MetacartaException
	{
	 	...
	 	while (st.hasMoreTokens())
		{
			...
			int status = LLDocs.ListObjects(vol, obj, null, filterString, LAPI_DOCUMENTS.PERM_SEECONTENTS, children);
			if (status != 0)
				throw new MetacartaException("Error finding start node path: "+llServer.getErrors());
			...
			throws MetacartaException
		{
			if (rootValue == null)
			{
				if (workspaceName.equals(CATEGORY_NAME))
					...
					throw new MetacartaException("Bad workspace name");
				if (rootValue == null)
					throw new MetacartaException("Could not get workspace/volume ID");
			}
